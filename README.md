...
新增企业级生产部署授权评估接口，统一校验制品签名、自动化测试、安全扫描、变更审批、冻结窗口例外、数据库迁移、回滚演练、错误预算、职责分离与审计证据；灰度、可观测性和业务通知未就绪时进入人工复核，不允许静默带风险上线。实现及调用方式见[生产部署授权说明](docs/ENTERPRISE_PRODUCTION_DEPLOYMENT.md)。
