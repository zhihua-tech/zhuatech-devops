...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `releaseNo` | String | 发布单号 |
| `testPassRate` | double | 测试通过率(%) |
| `criticalVulnerabilities` | int | 严重漏洞数 |
| `deploymentSuccessRate` | double | 部署成功率(%) |
| `changeFailureRate` | double | 变更失败率(%) |
| `leadTimeHours` | int | 交付前置时间(小时) |
| `approvalPassed` | boolean | 发布审批通过 |
| `rollbackReady` | boolean | 回滚方案就绪 |
