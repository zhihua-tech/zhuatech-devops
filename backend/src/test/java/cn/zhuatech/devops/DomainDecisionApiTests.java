...
@SpringBootTest @AutoConfigureMockMvc class DomainDecisionApiTests {
 ...
 @Test void domainDecisionReturnsAuditableScoreMetricsAndActions() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"releaseNo\":\"REL-2026-0088\",\"testPassRate\":99,\"criticalVulnerabilities\":0,\"deploymentSuccessRate\":99.5,\"changeFailureRate\":3,\"leadTimeHours\":18,\"approvalPassed\":true,\"rollbackReady\":true}"))
 ...
 @Test void domainRiskScenarioReturnsExpectedBlockingDecision() throws Exception {
  mvc.perform(post("/api/domain/decision").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"releaseNo\":\"REL-2026-0088\",\"testPassRate\":40,\"criticalVulnerabilities\":8,\"deploymentSuccessRate\":60,\"changeFailureRate\":50,\"leadTimeHours\":200,\"approvalPassed\":false,\"rollbackReady\":false}"))
